  ...
  return (
    <div className="plants-profile" style={{ margin: '1rem' }}>
      ...
      <div className="info">
        ...
        <div className="name" style={{color: data.repottingDueInDays < 0 ? 'red': 'black'}}> Repotting {data.repottingDueInDays < 0 ? ' overdue by': ' due in'} {data.repottingDueInDays < 0 ? data.repottingDueInDays * -1 : data.repottingDueInDays } days <button onClick={repot}>Update</button></div>
      </div>
    </div>
  );
}
